...
pub enum NodeState {
    ...
    /// [`LoadBalancingStrategy`] decides to ignore them. Since the driver does not connect to them,
    /// the only way it can assess their states is from topology events.
    Unknown,
    ...
    /// node (because it's ignored by the [`LoadBalancingStrategy`]), but it has received a topology
    ...
    /// actively trying to connect to the node (because it's ignored by the [`LoadBalancingStrategy`],
